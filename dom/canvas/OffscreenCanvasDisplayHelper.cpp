...
bool OffscreenCanvasDisplayHelper::MayUpdateContext(WorkerPrivate* aWorker, ErrorResult& aRv) {
  MutexAutoLock lock(mMutex);
  if (!mOffscreenCanvas) {
    return true;
  }
  ...
  aRv.ThrowInvalidStateError("Cannot update created context to different thread");
  return false;
}
